...
public class AuthenticationManger: NSObject {
    @objc static let shared = AuthenticationManger()
    private var authenticationCompletionHandlers: [String: (Result<AppleAccountData, Error>) -> Void] = [:]
    private var authenticationDataTimers: [String: Timer] = [:]
...
//                name: Notification.Name("de.tu-darmstadt.seemoo.OpenHaystack.AnisetteDataResponse"), object: nil)
    ...
    func requestAnisetteData(_ completion: @escaping (Result<AppleAccountData, Error>) -> Void) {
        if let accountData = self.requestAnisetteDataAuthKit() {
            os_log(.debug, "Authentication Data loaded %@", accountData.debugDescription)
